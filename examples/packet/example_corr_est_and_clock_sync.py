...
class example_corr_est_and_clock_sync(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.hdr_const = hdr_const = digital.constellation_qpsk().base()

        self.eb = eb = 0.22
        self.rxmod = rxmod = digital.generic_mod(hdr_const, False, sps, True, eb, False, False)
        self.nfilts = nfilts = 32
        self.access = access = digital.packet_utils.default_access_code
        ...
        self.ac = ac = [''.join(x) for x in zip(access[0::2], access[1::2])]
        ...
        self.digital_corr_est_cc_0 = digital.corr_est_cc((modulated_sync_word), sps, 0, 0.999)
        ...
        self.blocks_vector_source_x_0_0 = blocks.vector_source_b([42,32,34,65,77,34,65,23,54]+[0xac, 0xdd, 0xa4, 0xe2, 0xf2, 0x8c, 0x20, 0xfc], True, 1, [])
    ...
    def set_access(self, access):
        self.access = access
        self.set_ac([''.join(x) for x in zip(self.access[0::2], self.access[1::2])])
